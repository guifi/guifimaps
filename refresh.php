<?php
  ...
  $hlinks = @fopen(getenv('GUIFI_WEB')."/guifi/gml/".$rootZone."/links/csv", "r") or die("Error getting links cv\n");;
  if ($hlinks) {
   while (!feof($hlinks)) {
       print_r($member);
...
<ogr:FeatureCollection
  ...
  $hnodes = @fopen(getenv('GUIFI_WEB')."/guifi/gml/".$rootZone."/nodes/csv", "r");
  if ($hnodes) {
   while (!feof($hnodes)) {
